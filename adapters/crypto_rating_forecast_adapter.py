...
def cripto_rating_btc_forecast_grabber(last_rec_dy):
    """
    The changelly website provides bitcoin forecast price values with multiple tables.
    As of 1/5/2023, There is one table and one chart as follows.

    Table 1 = Today's overall metrics
    Graph 1 = Hourly forecast for 24 hours

    :param last_rec_dy: the last day processed to check whether to need to process
    :return: (today, hourly)
    """
    Crypto = 'BTC'
    url = 'https://www.crypto-rating.com/crypto-currencies/btc/'

    tdy_idx, hrly_idx = 0, 1
    ...
    # Find the script tag containing the desired Highcharts.chart
    pattern = re.compile(r'Highcharts.chart\(\s*\'price24\',\s*({.*?})\);', re.DOTALL)
    script_tags = soup.find_all('script', string=pattern)

    # Extract and process each matching script tag
    for script_tag in script_tags:
        script_content = script_tag.text

        data_match = pattern.search(script_content)
        ...
        if data_match:
            ...
            try:
                data_array = eval_js(f'JSON.stringify({chart_data}.series[0].data);')
                parsed_data = json.loads(data_array)

                # Now you can process the data_array as needed
                for item in parsed_data:
                    time, price = item[0], item[1]
                    print(f'Time: {time}, Price: {price}')
            except Exception as e:
                print(f'Error evaluating JavaScript: {e}')
        else:
            print('Data not found in the script tag.')


    # Initialize outputs
    today = hourly = pd.DataFrame()
...
def cripto_rating_forecast_grabber(Crypto, last_rec_dy):
    if Crypto == 'BTC':
        return cripto_rating_btc_forecast_grabber(last_rec_dy)


def check_db(Crypto):
    # qry1 = """
    # SELECT max(Date_Pred) as last_rec_day
    # FROM CrytoRatingDailyMetrics
    # where Crypto = '{}'
    # """.format(Crypto)
...
def process_cripto_rating_forecast():
    for Crypto in ['BTC']:
        last_rec_dy = check_db(Crypto)
        today, daily, annual, monthly = cripto_rating_forecast_grabber(Crypto, last_rec_dy)
        upload_to_db(today, daily, annual, monthly)


if __name__ == '__main__':
    ut.time_to_run(process_cripto_rating_forecast)
